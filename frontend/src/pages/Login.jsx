...
export default function Login() {
  ...
  // Form verileri (Login için email+pass, Register için hepsi lazım)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        if (userData && userData.balance === undefined) userData.balance = 0; 
        ...
        // --- 📝 KAYIT İŞLEMİ (USERNAME + EMAIL + ŞİFRE) ---
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', fontFamily: 'sans-serif' }}>
      <div style={{ backgroundColor: 'white', padding: '40px', borderRadius: '20px', boxShadow: '0 15px 35px rgba(0,0,0,0.3)', width: '380px', textAlign: 'center' }}>
        ...
        <h4 style={{ color: '#666', marginBottom: '30px' }}>{isLogin ? 'Email ile Giriş Yap' : 'Kayıt Ol ve Kazan'}</h4>
        
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          
          {/* KULLANICI ADI: Sadece Kayıt Olurken görünmeli */}
          {!isLogin && (
            <input 
              type="text" 
